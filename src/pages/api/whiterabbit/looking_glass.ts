...
import type { Glass, Token } from '../../../models/glass';
...
const MIN_UPDATE_INTERVAL = 14 * 60 * 1000; // 14 minutos em milissegundos
...
function isStable(token: RawToken | null | undefined): boolean {
  ...
  if (price >= 0.9 && price <= 1.1) return true;
  return false;
}

function isWrapped(token: RawToken | null | undefined): boolean {
  if (!token?.id) return false;
...
function filterTokens(tokens: RawToken[]): RawToken[] {
...
async function fetchTokens(perPage: number, order: string): Promise<RawToken[]> {
...
function cleanTokenData(token: RawToken): Token {
  return {
    id: token.id ?? '',
    ...
    percentChange24h: token.price_change_percentage_24h ?? 0,
    roi: token.roi,
  };
}

// NOVO: helpers de exclusividade de tokens nos blocos
function excludeUsedTokens(tokens: RawToken[], usedIds: Set<string>): RawToken[] {
  return tokens.filter(t => (t.id ? !usedIds.has(t.id) : false));
}

function processThresholdTokens(
  tokens: RawToken[],
  targetCount: number,
  isBest: boolean,
  usedIds: Set<string>
): { list: RawToken[]; threshold: number } {
  ...
  for (const threshold of thresholds) {
    let filtered = tokens.filter(t => (t.current_price ?? 0) < threshold && (t.id ? !usedIds.has(t.id) : false));
...
export interface RunLookingGlassOptions {
  force?: boolean;
}

export async function runLookingGlass({ force = false }: RunLookingGlassOptions = {}): Promise<Glass> {
  ...
  if (!force && timeSinceLastUpdate <= MIN_UPDATE_INTERVAL) {
    const error: NodeJS.ErrnoException & { statusCode?: number; meta?: Record<string, unknown> } = new Error(
      `Too soon to update. Last update was ${Math.floor(timeSinceLastUpdate / 1000)} seconds ago. Wait at least ${
        MIN_UPDATE_INTERVAL / 1000
      } seconds.`
    );
    error.statusCode = 429;
    error.meta = { lastUpdated: lastUpdated?.toISOString(), retryAfter: MIN_UPDATE_INTERVAL - timeSinceLastUpdate };
    throw error;
  }
  ...
  const sortedByPerformance = [...performancePool].sort(
  ...
  const positives = sortedByPerformance.filter(t => (t.price_change_percentage_24h ?? 0) > 0).length;
  const positiveTop12 = positives > 12 ? 12 : positives;
  ...
  const glassData: Glass = {
    ...
    marketStatus:
      positives < 12 ? 'Market down: few positives, filled with least negatives' : 'Market OK: all positives',
    counts: {
  ...
  await collection.updateOne({}, { $set: glassData }, { upsert: true });
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    const glassData = await runLookingGlass({ force });
    return res.status(200).json(glassData);
  } catch (error: unknown) {
    const err = error instanceof Error ? error : new Error('Internal server error. Please try again later.');
